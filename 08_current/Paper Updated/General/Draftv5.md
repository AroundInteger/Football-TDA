...
For persistent homology computation, each match is divided into temporal windows. For temporal analysis (Table 3 and Figure 3), 36 non-overlapping 2-minute windows (1,200 frames each at 10 Hz) are used for the primary match. For H1 loop counting (Table 1) and sensitivity analyses, 150 uniformly sampled frames (every 100th complete frame) are used, providing the same sampling basis as the multi-match validation.
...
| Team | 30.0 m (SkillCorner sweep value) | 100% of frames | 1–2 | 1.00 |

"Validation" denotes the cross-epoch consistency rate: the fraction of analysis windows across four temporal epoch lengths in which H0 falls within the domain-informed expected range for that regime. The individual (2.98 m) and team (30.0 m) cutoffs emerge from automated clustering quality metrics; for the SkillCorner broadcast data the Calinski–Harabasz optimum for the individual scale is 1.39 m, but we retain 2.98 m for cross-dataset comparability with the optical-tracking pilot and because the SkillCorner optimum reflects sub-metre positional noise rather than genuine player-proximity structure (see Section 4.4). The tactical cutoff (12.0 m) is selected as a domain-informed choice within the tactical range, where automated metrics diverge (silhouette-optimal: 16.31 m; information-content-optimal: 6.87 m). The value 12.0 m corresponds approximately to one-half the standard football zone width (the distance between the 18-yard box and the halfway line divided into quarters), and was validated on 50 single-frame analyses (96% producing expected H0). Sensitivity analysis (Section 3.6) confirms that H1 detection is operative across the 6–14 m range, with the chosen 12.0 m at the conservative end of this range.
...
**Justification of P75.** Empirical ablation across percentiles P50, P60, P75, P90, and P95 on the same 150-frame uniform subsample as Table in §3.2 (Section 3.6) shows that all percentiles produce identical detection counts (18 loops, 12.0% presence), confirming that the formula is insensitive to this parameter choice across the full tested range. The 75th percentile provides a robust but not overly permissive threshold. The 5.0 m floor prevents degenerate filtration at very small scales, and the 2δ factor ensures the filtration encompasses at least the inter-cluster distance regime. The persistence diagram, plotting (birth, death) coordinates of H1 features above the diagonal, is produced at this stage; Figure 1 shows a schematic H1 diagram (inset) at this step.
...
**Single-match (primary).** Across 150 analysis frames, the framework detects 432 H1 loops:

| Scale | Total loops | Frames with loops | Mean loops/frame | Mean persistence | Max persistence |
|-------|-------------|-------------------|-----------------|-----------------|-----------------|
| Individual (2.98 m) | 414 | 144/150 (96.0%) | 2.76 | 1.832 ± 1.555 | 7.626 |
| Tactical (12.0 m) | 18 | 18/150 (12.0%) | 0.12 | 2.693 ± 1.880 | 6.338 |
...
*95% CIs are match-level bootstrap intervals (1,000 resamples; `03_football_analysis/paper_v5_revisions/bootstrap_multi_match_ci.py`).* The individual-scale H1 presence rate of 97.0% (s.d. 1.5%; 95% CI over matches 96.1%–97.9%) across 10 independent matches confirms that loop structures are a near-universal feature of competitive spatial systems at this scale. Tactical-scale H1 presence of 19.3% (s.d. 7.2%; 95% CI over matches 15.3%–23.7%) shows meaningful cross-match variability, likely reflecting differences in tactical systems and match dynamics. The primary-match value of 12.0% (18/150 frames; Table in §3.2 above) sits within one standard deviation of this 10-match mean (|(12.0 − 19.3)/7.2| ≈ 1.01), consistent with ordinary between-match variability rather than the primary match being an outlier; the primary match was selected for its broadcast quality and complete event annotation, not its tactical H1 rate.

### 3.3 Closed Cycle Structures

Closed cycle identification recovers the geometric realisations of all 432 single-match H1 generators. Individual-scale cycles typically comprise 3–6 nodes (cluster centroids), corresponding to small ring-like player arrangements. Tactical-scale cycles typically comprise four or five nodes with higher persistence, corresponding to larger-scale formation gaps. See Figure 2 for a representative geometric realisation.

Representative examples (maximal-persistence feature per scale on the *n = 150* uniform subsample; sample indices need not coincide across scales):

- **Individual, sample frame 28**: 3-node cycle highlight from the maximal-persistence feature on a *k* = 19 centroid cloud (persistence 7.63 m).
- **Tactical, sample frame 35**: 5-node cycle geometric realisation (persistence 6.34 m), illustrating tactical-scale enclosure geometry.

![Figure 2: Geometric realisation of maximal-persistence H₁ loops at the individual (*δ* = 2.98 m) and tactical (*δ* = 12.0 m) scales for SkillCorner match 1996435 (individual panel: sample frame 28, *p* = 7.63 m over 19 centroids before cycle extraction; tactical panel: sample frame 35, *p* = 6.34 m, five centroid cycle vertices). Adaptive filtration ε_max per equation (1); *n* = 150 analysis frames uniformly subsampled.](figures/fig2_cycle_geometry.png)
...
Neither scale shows a statistically significant change between halves at the conventional α = 0.05 level. The tactical scale shows a modest non-significant increase (+13.5%, *p* = 0.428), whilst the individual scale shows a similarly non-significant increase (+5.7%, *p* = 0.358). This result is consistent with the multi-match finding (below) that temporal persistence dynamics are match-specific rather than universal.
...
We deliberately retain the coarse binary `half` factor here so that the LMM is directly comparable to the single-match Wilcoxon test above (§3.4 Table). A finer-grained treatment — continuous time-in-match, or phase-of-play covariates from the SkillCorner annotations — is deferred to the forthcoming functional persistence-landscape analysis (Bubenik, 2015; Chazal et al., 2015), where each match is treated as an element *t ↦ λ_δ(t)* of landscape space and regime transitions are the natural object of inference rather than half-averages.
...
The two scales capture weakly correlated but functionally complementary information. Across 900 frames from 6 matches (inclusion rule: the six matches in the 10-match cohort that have at least one frame with tactical-scale H1 > 0; four matches record zero tactical-scale H1 across all 150 sampled frames and are therefore uninformative for the cross-scale co-occurrence analysis; the six contributing match IDs are emitted to the summary JSON by `03_football_analysis/paper_v5_revisions/bootstrap_multi_match_ci.py` and ship in the supplementary):
...
The weak but significant positive correlation indicates that frames with more individual-scale loops tend to have slightly more tactical-scale loops, plausibly because denser player configurations create topological features at multiple scales. The scales show moderate positive co-occurrence (OR = 3.52) that falls short of conventional significance on the frame-level Fisher test (p = 0.093). The wide match-level bootstrap CI for the odds ratio [2.59, 13.53] reflects substantial between-match heterogeneity, and although its lower bound exceeds 1 it should not be read as a significant rejection of independence in the frequentist sense: we therefore treat the OR result as exploratory positive evidence of cross-scale co-occurrence pending full-season replication, and use the Spearman ρ = 0.254 as the primary quantitative evidence that the scales capture predominantly distinct structural information: individual-scale loops can appear without tactical-scale loops and vice versa.

*Comparative properties at the two H1 scales, primary match 1996435 unless otherwise stated; the final row gives the 10-match mean ± across-match s.d. for context.*

| Property | Individual scale | Tactical scale |
|----------|-----------------|----------------|
| Loop frequency (primary match) | High (2.76/frame) | Low (0.12/frame) |
| Mean persistence (primary match) | Low (1.832 ± 1.555 m) | High (2.693 ± 1.880 m) |
| Max persistence (primary match) | 7.626 m | 6.338 m |
| Frames with loops (primary match) | 144/150 (96.0%) | 18/150 (12.0%) |
| Frames with loops (10-match mean ± s.d.) | 97.0% ± 1.5% | 19.3% ± 7.2% |
...
These distances quantify *within-frame* complementarity between the individual-scale and tactical-scale persistence diagrams. Taking the mean individual-scale H1 persistence (1.832 m; §3.2) as a reference, the median bottleneck distance (1.511 m) is of the same order as a typical individual-scale feature, while the 95th-percentile tail (max 7.994 m) is an appreciable fraction of tactical-scale variability (mean 2.693 m across loops; §3.2). The landscape *L*² distances (median 5.671) confirm a non-trivial separation in functional space. Together these TDA-native summaries corroborate the complementarity claim in a metric that is independent of the Spearman/Fisher rank-based summaries above. *Between-tactical-class* distances (required for a fully powered landscape-*L*² comparison at population scale) are a distinct quantity, deferred to the forthcoming full-season analysis.

### 3.6 Sensitivity Analysis

**Cutoff distance sensitivity (tactical scale).** H₁ detection across δ ∈ {6, 8, 10, 12, 14, 16} m on the same 150-frame uniform subsample as §3.2:

| δ (m) | H₁ total | H₁ presence | Mean H₀ |
|-------|----------|-------------|---------|
| 6 | 289 | 88.0% | 13.6 |
| 8 | 167 | 69.3% | 10.1 |
| 10 | 64 | 36.0% | 7.0 |
...
H₁ detection decreases monotonically with cutoff distance, as expected: larger cutoffs produce fewer centroids and thus fewer potential cycles. The chosen δ = 12.0 m is deliberately at the conservative end of the productive range (12.0% frame presence vs 88.0% at δ = 6 m), prioritising fewer but more robust tactical loops, consistent with our single-linkage philosophy and domain-informed H₀ validation. The framework produces topological features across the operative tactical cutoff range (6–14 m); detection collapses to near-zero by δ = 16 m on this single-linkage, 150-frame primary-match subsample.

**Adaptive filtration ablation.** At δ = 12.0 m, varying the adaptive percentile on the same subsample:

| Percentile | H₁ total | H₁ presence | Mean filtration |
|-----------|----------|-------------|----------------|
| P50 | 18 | 12.0% | 40.2 m |
| P60 | 18 | 12.0% | 44.3 m |
| P75 | 18 | 12.0% | 51.5 m |
...
| Finish | −0.375 | < 0.001 | 720 |
| Off-ball run | −0.125 | < 0.001 | 4,882 |
...
The adaptive filtration formula addresses a practical coupling between the clustering and filtration steps. Ablation across the percentile in equation (1) shows that the formula is insensitive to this choice over the full tested range P50–P95: tactical-scale H₁ totals and frame-presence rates are identical (18 loops, 12.0%; Section 3.6), with only the implied filtration radius rising from 40.2 m at P50 to 66.4 m at P95. We report P75 as a conventional upper-quartile summary of inter-centroid distances rather than as an empirically optimal value: in the present sample any percentile in [P50, P95] is equally defensible, and the role of the percentile term is to track post-clustering geometry, while the max(5.0, 2δ) floor protects against degenerate filtration at small δ. This is a general solution applicable to any multi-scale TDA pipeline that uses clustering as a preprocessing step.
...
The temporal evolution result is nuanced: on the SkillCorner primaries plotted in Figure 3 (36 non-overlapping two-minute windows at 10 Hz), half-level mean tactical-scale persistence rises modestly but non-significantly (+13.5%; Mann–Whitney *p* = 0.428), and half-level mean individual persistence likewise increases non-significantly (+5.7%; *p* = 0.358). The direction is not consistent across validation matches.

The non-significant primary-match tests reflect limited half-level replication (*n* = 18 tactical windows labelled period 2 and comparable counts in period 1) together with stochastic window-to-window variability, rather than decisive evidence that halves are identical. Half-level persistence dynamics appear driven by match-specific factors such as tactical adjustments, scoreline effects, and substitutions rather than universal trends.
...
**Broadcast tracking resolution.** All data in this study are broadcast-derived (SkillCorner, 10 Hz). Surveys of tracking in team sports highlight trade-offs among sampling rate, spatial precision, and deployment cost (Cummins et al., 2013; Aughey, 2011); broadcast products typically offer lower spatial precision than higher-frequency optical systems (e.g. 25 Hz), which may affect persistence magnitude estimates. The framework's structural findings (scale regimes, H1 presence rates, sensitivity profiles) are expected to be robust across tracking technologies, but direct persistence magnitude comparison with optical data remains for future work. The Calinski–Harabasz optimum for the individual scale shifts from 2.98 m (optical tracking pilot) to 1.39 m (broadcast tracking), consistent with the lower positional precision of broadcast-derived player coordinates at sub-metre scales; we retain 2.98 m for cross-dataset comparability (Section 2.3).
...
**Code.** Implementation, analysis pipelines and figure generation scripts are available at `https://github.com/AroundInteger/Football-TDA`, tagged release `paper-v1.0` (the exact commit hash is recorded in the GitHub release notes and is cut at the time of manuscript submission). The multi-match validation pipeline corresponds to `03_football_analysis/multi_match_validation.py` with the `--skillcorner-only` flag; event correlation is reproduced by `real_event_correlation.py`. The Paper v5 revision additions are reproduced by the scripts in `03_football_analysis/paper_v5_revisions/`: `bootstrap_multi_match_ci.py` (Section 3.5 bootstrap CIs), `tda_native_distances.py` (Section 3.5 bottleneck/landscape distances), `half_level_random_effects.py` (Section 3.4 LMM), `event_window_sensitivity.py` (Section 3.8 sensitivity), `baseline_vs_topology.py` (Section 3.9 partial *R²*), `bilateral_topology.py` (Section 3.10 bilateral coupling), and `predictive_utility.py` (Section 3.11 held-out-match predictive utility).

**Software stack.** Python 3.11; Ripser.py 0.6.4 (Tralie et al., 2018); GUDHI 3.9 (bottleneck distance, landscapes); giotto-tda 0.6 (Tauzin et al., 2021) for cross-checks; NumPy 1.26, SciPy 1.12, pandas 2.1, scikit-learn 1.4, statsmodels 0.14. The repository includes a `Dockerfile` and an Apptainer definition with pinned versions; a ready-built image is published alongside the `paper-v1.0` tag to enable bit-for-bit reproduction without reinstalling the toolchain.

**Synthetic exemplar.** A synthetic 22-agent swarm with seeded ring formations is included in the code release to enable teaching and reproduction without proprietary tracking data. Acceptance criterion: individual-scale H₁ presence rate within ±3 pp of the primary match (96.0%), tactical-scale H₁ presence rate within ±5 pp of 12.0%, and at least one H₁ generator per frame recovered by closed-cycle identification for all frames with non-zero H₁. A pass/fail test implementing these checks ships in the code release.
...
Buldú, J.M., Busquets, J., Echegoyen, I. and Seirul·lo, F. (2019). Defining a historic football team: using network science to analyse Guardiola's FC Barcelona. *Scientific Reports*, 9, 13602.
...
Clemente, F.M., Couceiro, M.S., Martins, F.M.L. and Mendes, R.S. (2015). Using network metrics in soccer: a systematic review. *Journal of Human Kinetics*, 45, 123–134.
...
Duarte, R., Araújo, D., Correia, V. and Davids, K. (2012). Sports teams as superorganisms: implications of sociobiological models of behaviour for research and practice in team sports performance analysis. *Sports Medicine*, 42(8), 633–642.
...
Folgado, H., Lemmink, K.A.P.M., Frencken, W. and Sampaio, J. (2014). Length, width and centroid distance as measures of teams' tactical performance in youth football. *European Journal of Sport Science*, 14(S1), S487–S492.
...
SkillCorner. (2024). SkillCorner open broadcast tracking data. <https://github.com/SkillCorner/opendata> (accessed 2026).

Tauzin, G., Lupo, U., Tunstall, L., Pérez, J.B., Caorsi, M., Medina-Mardones, A.M., Dassatti, A. and Hess, K. (2021). giotto-tda: A topological data analysis toolkit for machine learning and data exploration. *Journal of Machine Learning Research*, 22(39), 1–6.
...
Tralie, C., Saul, N. and Bar-On, R. (2018). Ripser.py: A lean persistent homology library for Python. *Journal of Open Source Software*, 3(29), 925.
...
**Target Journal**: Journal of Applied and Computational Topology (JACT)
