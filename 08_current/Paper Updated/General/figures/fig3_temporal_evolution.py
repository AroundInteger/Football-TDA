...
Run from Football-TDA root: python 08_current/paper/figures/fig3_temporal_evolution.py
"""
...
def main():
    ...
    df = df[df['match_id'] == PRIMARY_SKILLCORNER].copy()
    if df.empty:
        raise SystemExit(
            f"No rows for SkillCorner primary match ID {PRIMARY_SKILLCORNER} in fig2_temporal.csv",
        )
