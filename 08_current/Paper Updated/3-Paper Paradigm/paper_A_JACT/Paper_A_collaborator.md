...
Addressing Q3, the two levels are not measuring the same structure at different resolutions. If the tactical signal were a coarsened version of the individual signal, the two would co-occur far more consistently than Table 5 shows. The bottleneck and landscape distances would also be small relative to each level's own filtration lifetimes (Section 3.5). Clustering before persistent homology is therefore not a convenience for separating noise. It produces two informationally distinct objects.

### 4.4 Construct validity

Addressing Q4, the preliminary answer is yes: total persistence is not independent of on-ball events. At the individual level, mean $\Delta$ is $-0.253$ m after on-ball engagements and $+0.700$ m after build-up (Section 3.6). That is a construct-validity check only. Multiple-testing control and football interpretation belong to a companion manuscript (Paper B).
