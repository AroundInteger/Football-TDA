...
$H_1$ detection is not confined to the adopted cutoff or to a single
truncation percentile (Section~\ref{sec:sensitivity}). The operative
setting therefore occupies a band, not an isolated optimum. Application
to another bounded competitive system still requires the interaction
lengths to be re-derived on that domain.
...
of the ten matches (Figure~\ref{fig:cutoff}). Those regimes likewise
persist over a range of $\delta$ rather than appearing only at the
adopted cutoffs.
