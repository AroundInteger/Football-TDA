...
Addressing question~\ref{q:levels}, we recover the three
organisational levels directly from the data. Throughout, $H_0$ is the
number of cluster centroids. Equivalently, it
...
inside the acceptance intervals $[15,22]$, $[4,10]$, and $[1,2.5]$
...
about $1.6$ clusters merge per metre from the individual to the
tactical level, but only about $0.27$ per metre from the tactical to
the team level, so the two surviving envelopes remain over a wide band
of $\delta$ (Figure~\ref{fig:cutoff}a). The spatial realisation on a
single frame (Figure~\ref{fig:cutoff}d--f) shows the same three
regimes: a near-atomic cloud at $2.75$~m, a handful of coordinating
groups at $11.75$~m, and two spatial envelopes at $23.0$~m, which are
topological components rather than the two teams.

\subsection{\texorpdfstring{$H_1$}{} loop detection}
\label{sec:h1detection}

Addressing question~\ref{q:homology}, we test whether
scale-attributable homology detects loop structure at those levels. On
...
Figure~\ref{fig:cycles} realises the bar of largest $p$ on sample
frame~95 as a loop in space.
Figure~\ref{fig:h1bars} shows that bar on the persistence diagram,
...
The tactical level sits close to a cardinality floor. In the ten-match sample, 40.6\% of frames have four or fewer
...
\caption{Scale-separated finite \texorpdfstring{$H_1$}{} bars on the
primary match (uniform 150-frame sample).
...
The individual-level maximum in Table~\ref{tab:h1single} (frame~141) is
a near-collinear sliver and is not the cycle shown in
...
Addressing the robustness part of question~\ref{q:homology}, we vary
the cutoff and the truncation. Tactical $H_1$ counts fall monotonically
...
Addressing question~\ref{q:distinct}, we ask whether the two levels
report the same structure. The two $H_1$ levels carry related but
largely distinct information.
...
(3.778~m mean filtration lifetime where loops appear;
...
Addressing question~\ref{q:validity}, and as a sanity check against
measurement noise, we asked whether total persistence moves with real
match events (SkillCorner annotations; 103{,}856 event--topology pairs
across ten matches). Write $\Delta$ for the change in mean total
persistence from the five 1~Hz frames before the event to the five
after it (Section~\ref{sec:stats}). On-ball engagements precede a drop
at the individual level (mean $\Delta=-0.253$~m, $n=8{,}927$,
Mann--Whitney $p<10^{-13}$). Build-up precedes a rise (mean
$\Delta=+0.700$~m, $n=618$, $p<10^{-6}$). Both classes move in the same
direction at the tactical level. These are nominal tests on
pre-specified classes. We treat the result as a preliminary
construct-validity check only. Multiple-testing control and football
interpretation belong elsewhere.
