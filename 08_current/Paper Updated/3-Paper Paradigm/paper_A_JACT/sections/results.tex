...
Addressing question~\ref{q:levels}, the homology sample reproduces
the three organisational levels inverted from the data in
Section~\ref{sec:cutoffselection}. Those levels are individual
($\delta = 2.75$~m), tactical ($\delta = 11.75$~m), and team
($\delta = 23.0$~m). Throughout, $H_0$ is the
...
related but largely distinct information.
...
Total persistence moves with
on-ball match events. This is a construct-validity check against
measurement noise, not a football analysis. The comparison uses
SkillCorner annotations (103{,}856 event--topology pairs across ten
matches). Write $\Delta$ for the change in mean total
