...
complete frame (1~Hz). Supplementary Figure~\ref{fig:acf} is an
...
Complete-linkage and Ward's method are not used; their effect on $H_1$
counts is examined in Section~\ref{sec:limitations}.
...
A complete frame has $n=22$ agents. The grid is
$\delta\in[0.25, 40.0]$~m at $0.25$~m steps (160 points), and every
complete 22-player frame of the ten matches in
Supplementary Table~\ref{tab:matches} is used (436{,}648 frames), with
no random window draw and no mixing of epoch lengths.
...
finite $H_1$ bars. Three summaries of those bars appear in the results
and must not be read as clock time. \emph{Presence} is the fraction of
...
event to the five immediately after it (five seconds either side; the
event frame itself is excluded).
Event classes are pre-specified. The $p$-values are nominal.
