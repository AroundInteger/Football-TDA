...
**TDA Football Paper B note:** Ten-match observational design with match-grouped cross-validation. Do not demand single-study power calculations at clinical-trial standard. Flag random frame-level CV for temporal tracking data as [CRITICAL]. See `review-tda-football-papers.md` §4.2–4.3.
...
**TDA Football pipeline (when applicable):** Apply the checklist in `review-tda-football-papers.md` §5 — Python scripts under `02_tda_core/` and `03_football_analysis/`, CSV outputs, and `grant/CANONICAL_NUMBERS.md` for cross-document numeric alignment.
...
**Split-paper scope creep (PEF, TDA Football, and similar projects)**
When theory/method and application validation are split across linked manuscripts, check that each paper claims only what it owns. See `review-pef-papers.md` §3 (PEF) or `review-tda-football-papers.md` §3 (TDA Football). Cross-cite discipline failures are often [CRITICAL].
