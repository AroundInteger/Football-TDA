...
### 5.6 TDA Football split publication (`paper_A_JACT` + `paper_B_JSS`)

When either manuscript is under review, read `review-tda-football-papers.md` in full. Summary:

| Paper | Owns | Must not headline |
|---|---|---|
| **Paper A (JACT)** | Scale decomposition, adaptive filtration, multi-match H₀/H₁ validation, complementarity, sensitivity | Event interpretation, geometric baselines, bilateral coupling, predictive utility (brief validity check only) |
| **Paper B (JSS)** | Event correlation, baseline comparison, bilateral independence, predictive null | Full method re-derivation, H₀/H₁ regime tables |

**Default venues:** Paper A → **JACT**; Paper B → **JSS** (see `working_foundations.md`).

**Grant alignment:** EPSRC Vision & Approach aligns to Paper A only; cross-check `grant/CANONICAL_NUMBERS.md`.

**Primary methodological community vs application domain:** JACT reviewers weight framework rigour and robustness; JSS reviewers weight practitioner interpretation and honest predictive limits.
...
**TDA Football venue notes (detail in `review-tda-football-papers.md` §7):**

| Venue | Paper | Reviewer emphasis |
|---|---|---|
| **JACT** | Paper A | Methodological rigour; robustness; domain-agnostic framing; reproducibility |
| **JSS** | Paper B | Practitioner interpretation; table-first reporting; honest predictive limits |
...
TDA Football fields (when applicable):
Paper role:       [Paper A JACT | Paper B JSS | joint]
Sibling paper:    [in prep | arXiv posted | submitted | n/a]
Grant alignment:  [checked | not checked | n/a]
Pipeline status:  [figures complete | placeholders remain | numbers synced]
Reference docs:   [working_foundations.md, CANONICAL_NUMBERS.md]
```
