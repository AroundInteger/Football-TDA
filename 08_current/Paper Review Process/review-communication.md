...
**TDA Football Paper A (JACT):** Apply `abstract-structure-guide-tda.md` §Paper A. Flag abstracts over 250 words, equations, or football-practitioner framing as [SIGNIFICANT].

**TDA Football Paper B (JSS):** Apply `abstract-structure-guide-tda.md` §Paper B. Flag dense inline statistics, methodology re-derivation, or failure to state predictive limit as [SIGNIFICANT].

**TDA Football house style (both):** Flag em dashes in prose, rhetorical questions, and vignettes as [SIGNIFICANT] per `working_foundations.md` §2.
