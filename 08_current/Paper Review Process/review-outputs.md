...
| D — Joint consistency | PEF empirical + companion **or** TDA Football A + B | **Joint format** in project annex §8 (`review-pef-papers.md` or `review-tda-football-papers.md`) |
...
For TDA Football manuscripts in Mode A, include scope/pipeline findings from `review-tda-football-papers.md` in Format A under Major Concerns as appropriate.
...
**TDA Football-specific examples (see `review-tda-football-papers.md`):**
- Paper A cites Paper B outside Discussion → Outlook
- Paper B re-derives adaptive filtration or full H₁ tables instead of citing Paper A
- Manuscript numbers contradict `CANONICAL_NUMBERS.md` or Results tables
- Paper B predictive section uses frame-random CV instead of match-grouped CV
- Submission blockers: missing GitHub URL, EPSRC ref, or Springer/JSS template transfer incomplete
