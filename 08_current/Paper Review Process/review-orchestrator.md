...
3. `review-tda-football-papers.md` — **when reviewing `paper_A_JACT` or `paper_B_JSS`** (after calibration, before science)
4. `review-science.md` — always required (scientific rigour + numerical pipelines)
5. `review-communication.md` — always required (writing + visuals)
6. `review-outputs.md` — always required (output format selection + report generation)

Do not skip any sub-skill. Each encodes non-overlapping expertise. Skip project annexes only for documents outside PEF and TDA Football split-publication projects.
...
3. If TDA Football paper: run `review-tda-football-papers.md` checklists alongside science audit
4. Run `review-science.md` → audit scientific rigour and numerical integrity
5. Run `review-communication.md` → audit writing and visual communication
6. Run `review-outputs.md` → select report format, apply severity triage, generate structured output
7. End with a prioritised action list (see §5 below)
...
### Mode D — Joint consistency review (split-publication projects)
**Trigger:** User requests a cross-paper pre-submission check, or both manuscripts in a split-publication pair are in scope:
- **PEF:** `pef-empirical` + `pef-mathematics`
- **TDA Football:** `paper_A_JACT` + `paper_B_JSS`

**Sequence:**
1. Run brief dual calibration (both paper roles and venues)
2. Run the applicable project annex: `review-pef-papers.md` §3, §5, §6 **or** `review-tda-football-papers.md` §3, §5, §6
3. Produce output using the **Joint consistency format** in the project annex (§8)
4. Do **not** run full Mode A science/communication audits unless the user explicitly requests them
5. End with cross-paper priority items only; recommend which paper to full-review next (empirical / Paper A first)
...
- Mode D joint consistency (PEF or TDA Football): focused cross-paper report only; typically shorter than Mode A
