# Vision and Approach — EPSRC criteria structure (V5.1)

<!-- Version: V5.1 — structural-hinge restore on V5 (`VA_V5_EPSRC_structure.md`).
     Factual compression of V5 is unchanged. This pass restores only short
     topic/signpost sentences that help skim-reviewers track argument position
     against EPSRC criteria (~80–120 words), without reintroducing second
     tellings of the same facts.

     Restored hinges:
     (1) §2 — three-foundation opener + light “second/third foundation” labels.
     (2) §3 — pilot-known → population-open bridge.
     (3) §4 — labelled Need beat (no Opportunity paraphrase).
     (4) §5/§6 — slightly fuller topic clauses (still one claim each).
     (5) §7 — Vision→Objectives bridge mapping O1–O3 onto §3 (i)–(iii).

     Target: ≤3 pages Calibri 11 pt, 2 cm margins (JeS paste).
     ...
     Compressed baseline: VA_V5_EPSRC_structure.md
...
Section 2 shows that scale-specific barcodes exist, are reproducible, and respond coherently to match events at 10-match scale. The open question — and the new knowledge this project generates — is what holds at *population* scale (~540 Championship matches). **(i) Distributional laws:** do pilot stability scores, H₁ presence rates and complementarity behave as population regularities, or vary with tactical context? **(ii) Comparison geometry:** landscape [6] and image [9] distances that distinguish formation systems in persistence space. **(iii) Functional dynamics:** within-match paths *t* ↦ λ_δ(*t*) characterising stability regimes and transitions. Establishing (i)–(iii) is this project's contribution to knowledge (Objectives O1–O3, §7).

**4. Timeliness, Need and Opportunity**

**Timeliness.** Three developments now converge: survey-level consolidation of multiparameter persistence [4] and statistical TDA with convergence guarantees [6,7,9]; computational tooling (Ripser [23], GUDHI, giotto-tda [24]) that makes per-frame homology tractable at season scale; and, to our knowledge, newly accessible season-scale competitive tracking data.

**Need.** No validated workflow yet exists for multi-scale persistent homology on competitive, hierarchical, high-frequency systems — the methodological and applied need this project answers.

**5. Impact**

**Impact on research.** The primary impact is to the mathematical sciences: a validated open-source library for multi-scale persistent homology on competitive, time-evolving point clouds, contributing to EPSRC's *physical and mathematical sciences powerhouse* priority and building UK capability in applied topology, with links to *AI, digitisation and data*.

**Economic and industry impact.** Co-developed with Swansea City AFC and data partners (StatsBomb, Genius Sports), the project translates topological measures into practitioner-interpretable tools for pressing structures, formation gaps and defensive-line organisation that geometric descriptors miss.
...
**Direct beneficiaries:** the mathematical sciences community (open-source library); Swansea City AFC analysts as co-development partner; and the wider sports-analytics research community. **Longer-term:** operators of other competitive multi-agent domains, once characteristic scales are re-derived; and the proposed Standard Grant programme (§7, O4).
...
*Note: numbers follow the V4.1 / compiled LaTeX order (unchanged in V5–V5.1). Spot-check against the compiled bibliography before JeS paste.*
