...
\textbf{Single-match (primary).} Across 150 analysis frames, the framework detects 403 \(H_1\) loops:
...
The individual-scale \(H_1\) presence rate of 97.0\% (s.d.\ 1.5\%; 95\% CI over matches \(96.1\%\)--\(97.9\%\)) across 10 independent matches confirms that loop structures are a near-universal feature of competitive spatial systems at this scale. Tactical-scale \(H_1\) presence of 19.3\% (s.d.\ 7.2\%; 95\% CI over matches \(15.3\%\)--\(23.7\%\)) shows meaningful cross-match variability, likely reflecting differences in tactical systems and match dynamics. The primary-match value of 12.7\% (19/150 frames; Table~\ref{tab:h1single}) sits within one standard deviation of this 10-match mean (\(|(12.7 - 19.3)/7.2|\approx 0.92\)), consistent with ordinary between-match variability rather than the primary match being an outlier; the primary match was selected for its broadcast quality and complete event annotation, not its tactical \(H_1\) rate.

\subsection{Closed Cycle Structures}
\label{sec:closed_cycles}

Closed cycle identification recovers the geometric realisations of all 403 single-match \(H_1\) generators. Individual-scale cycles typically comprise 4--6 nodes (cluster centroids), corresponding to small ring-like player arrangements. Tactical-scale cycles comprise 4--5 nodes with higher persistence, corresponding to larger-scale formation gaps. See Figure~\ref{fig:cycles} for a representative geometric realisation.

Representative examples:
\begin{itemize}
  \item \textbf{Individual, Frame 50:} 5-node cycle, persistence 12.99~m, a tight pentagonal pressing arrangement.
  \item \textbf{Tactical, Frame 50:} 4-node cycle, persistence 10.77~m, the highest-persistence feature, corresponding to a stable gap between defensive and midfield lines.
\end{itemize}

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/fig2_cycle_geometry.\figext}
  \caption[Geometric realisation of \(H_1\) cycles]{Geometric realisation of representative \(H_1\) loops at individual (\(\delta=2.98\)~m) and tactical (\(\delta=12.0\)~m) scales for SkillCorner match 1996435, frame 50. Loops are the highest-persistence cycle per scale (individual: 5-node, persistence 12.99~m; tactical: 4-node, persistence 10.77~m), recovered from the adaptive filtration with \(\varepsilon_{\max}\) per equation~\eqref{eq:adaptive}. Single-match sample: \(n=150\) analysis frames (uniform, every 100th).}
  \label{fig:cycles}
\end{figure}
...
Neither scale shows a statistically significant change between halves at the conventional $\alpha=0.05$ level. The tactical scale shows a large but non-significant increase (+78.8\%, $p=0.066$), whilst the individual scale shows a small non-significant increase (+9.5\%, $p=0.506$). This result is consistent with the multi-match finding (below) that temporal persistence dynamics are match-specific rather than universal.
...
We deliberately retain the coarse binary \texttt{half} factor here so that the LMM is directly comparable to the single-match Wilcoxon test of Table~\ref{tab:temporal}. A finer-grained treatment --- continuous time-in-match, or phase-of-play covariates from the SkillCorner annotations --- is deferred to the forthcoming functional persistence-landscape analysis \citep{Bubenik2015,Chazal2014}, where each match is treated as an element \(t \mapsto \lambda_\delta(t)\) of landscape space and regime transitions are the natural object of inference rather than half-averages.
...
The two scales capture weakly correlated but functionally complementary information. The analysis is computed across 900 frames from six of the ten matches in Table~\ref{tab:h1multi} (inclusion rule: at least one sampled frame with tactical-scale $H_1 > 0$; four matches record zero tactical-scale $H_1$ across all 150 sampled frames and are therefore uninformative for the cross-scale co-occurrence analysis). The six contributing match IDs are emitted to the summary JSON by \texttt{paper\_v5\_revisions/bootstrap\_multi\_match\_ci.py} and ship in the supplementary material:
...
The weak but significant positive correlation indicates that frames with more individual-scale loops tend to have slightly more tactical-scale loops, plausibly because denser player configurations create topological features at multiple scales. The scales show moderate positive co-occurrence (OR \(=3.52\)) that falls short of conventional significance on the frame-level Fisher test (\(p=0.093\)). The wide match-level bootstrap CI for the odds ratio \([2.59,\,13.53]\) reflects substantial between-match heterogeneity, and although its lower bound exceeds 1 it should not be read as a significant rejection of independence in the frequentist sense: we therefore treat the OR result as exploratory positive evidence of cross-scale co-occurrence pending full-season replication, and use the Spearman \(\rho=0.254\) as the primary quantitative evidence that the scales capture predominantly distinct structural information: individual-scale loops can appear without tactical-scale loops and vice versa.

\begin{table}[H]
  \centering
  \caption{Comparative properties at the two \(H_1\) scales. Primary match is 1996435; the final row reports the 10-match mean \(\pm\) across-match s.d.\ for frame presence for context.}
  \label{tab:complement}
  \begin{tabular}{@{}lcc@{}}
      \toprule
      Property & Individual scale & Tactical scale \\
      \midrule
      Loop frequency (primary match)      & High (2.55/frame)          & Low (0.14/frame) \\
      Mean persistence (primary match)    & Low (\(1.977\pm1.128\)~m)  & High (\(3.797\pm3.008\)~m) \\
      Max persistence (primary match)     & 12.991~m                   & 10.771~m \\
      Frames with loops (primary match)   & 143/150 (95.3\%)           & 19/150 (12.7\%) \\
      Frames with loops (10-match mean $\pm$ s.d.) & \(97.0\%\pm1.5\%\) & \(19.3\%\pm7.2\%\) \\
      \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[H]
  \centering
  \caption{TDA-native cross-scale distance distributions (1{,}500 frames, 10 matches). These distances quantify within-frame complementarity between the individual-scale and tactical-scale persistence diagrams; between-tactical-class distances (required for a fully powered landscape-$L^2$ comparison at population scale) are a distinct quantity, deferred to the forthcoming full-season analysis.}
...
These distances are interpreted with the mean individual-scale \(H_1\) persistence (1.977~m; Table~\ref{tab:h1single}) as a reference. The median bottleneck distance (1.511~m) is of the same order as a typical individual-scale feature length, while the 95th-percentile tail (max 7.994~m) approaches twice the tactical-scale mean persistence (3.797~m, Table~\ref{tab:h1single}). The landscape \(L^2\) distances (median 5.671) confirm a non-trivial separation in functional space. Together these TDA-native summaries corroborate the complementarity claim in a metric that is independent of the Spearman/Fisher rank-based summaries above.

\subsection{Sensitivity Analysis}
\label{sec:sensitivity}

\textbf{Cutoff distance sensitivity (tactical scale).} \(H_1\) detection across \(\delta\in[6,16]\)~m on 150 frames of the primary match ($H_1$ becomes zero at $\delta=16$~m, so the effective operative range is $[6,14]$~m):
...
\(H_1\) detection decreases monotonically with cutoff distance, as expected: larger cutoffs produce fewer centroids and thus fewer potential cycles. The chosen \(\delta=12.0\)~m is deliberately at the conservative end of the productive range (12.7\% frame presence vs 87.3\% at \(\delta=6\)~m), prioritising fewer but more robust tactical loops, consistent with our single-linkage philosophy and domain-informed \(H_0\) validation. The framework produces topological features across the operative tactical cutoff range (6--14~m); detection collapses to zero at \(\delta \geq 16\)~m for the single-linkage, 150-frame primary-match sample.
...
Geometric descriptors and tactical-scale \(H_1\) persistence are weakly-to-moderately associated (Spearman $|\rho|$ up to $0.43$ for convex-hull area); the partial \(R^2\) values show that topology explains non-negligible residual variance in the baselines after controlling for the other three (convex-hull area $0.091$; team width $0.036$; team length $0.025$), so tactical-scale \(H_1\) persistence is not reducible to these geometric descriptors alone. The largest pairing (hull area $0.091$) exceeds a $0.05$ effect-size threshold commonly used as a non-redundancy criterion in regression-based baseline comparisons; the partial \(R^2\) reported here is a symmetric proxy, not the fully-specified ANCOVA with topology as the tested block, which is the definitive test to be conducted at full-season scale. Section~\ref{sec:predictive} converts this correlational statement into a cross-validated predictive-utility claim on held-out matches.

\paragraph{Multiple testing and effect sizes.} With four Spearman tests, a Bonferroni-adjusted threshold of $\alpha/4 = 0.0125$ leaves team width, convex-hull area, and Voronoi dispersion entropy significant at the corrected level, and team length ($p=0.857$) non-significant, so the qualitative picture is unchanged. The extremely small $p$-values for team width ($<10^{-40}$) and hull area ($<10^{-65}$) are driven by the large per-frame sample size ($N=1{,}500$ frames pooled across 10 matches), not by large effect sizes: Spearman $|\rho|$ is at most $0.43$, and the largest partial $R^2$ is $0.091$, so the geometric and topological descriptors are not redundant but are also not strongly coupled on a per-frame basis.
...
constructed from the SkillCorner \texttt{team\_in\_possession\_phase\_type} annotation; the build-up class is the largest of the positively-signed event types in Section~\ref{sec:event_correlation} and yields a balanced-enough target across the 10-match cohort (prevalence \(13.7\%\) of frames). Two feature blocks are compared:
