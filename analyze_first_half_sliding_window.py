#!/usr/bin/env python3
"""
First Half Sliding Window TDA Analysis
=====================================

This script analyzes the first half of the SecondSpectrum match using sliding windows
to provide comprehensive coverage of formation evolution in the first 45 minutes.

Features:
- Sliding window analysis (5-minute windows, 1-minute steps)
- First half focus (0-45 minutes)
...
class FirstHalfSlidingWindowAnalyzer:
    """
    Analyzes the first half of the SecondSpectrum match using sliding windows
    """
    
    def __init__(self, data_file, window_size=7500, step_size=1500, max_filtration=2.0, max_workers=None):
        """
        Initialize the first half sliding window analyzer
        ...
        print(f"FirstHalfSlidingWindowAnalyzer initialized")
    ...
    def create_first_half_windows(self):
        """
        Create sliding window definitions for the first half (0-45 minutes)
        """
        print(f"\n=== Creating First Half Sliding Windows ===")
        
        # First half: 0 to 45 minutes (0 to 67,500 frames at 25Hz)
        first_half_end_frame = 45 * 60 * 25  # 67,500 frames
        
        print(f"First half: 0 - {first_half_end_frame} frames (0 - 45 minutes)")
        ...
        for start_frame in range(0, first_half_end_frame - self.window_size + 1, self.step_size):
        ...
        print(f"Created {len(self.windows)} sliding windows for first half")
    ...
    def analyze_all_windows_parallel(self):
        ...
        """
        print(f"\n=== Analyzing All First Half Windows in Parallel ===")
        print(f"Using {self.max_workers} parallel workers")
        
        if not self.windows:
            self.create_first_half_windows()
        ...
        print(f"\n✓ First half parallel analysis complete!")
    ...
    def create_comprehensive_analysis(self):
        ...
        """
        print(f"\n=== Creating First Half Comprehensive Analysis ===")
        ...
        print("First half comprehensive analysis created:")
    ...
    def export_results(self, output_dir='first_half_sliding_window_results'):
        """
        ...
        if hasattr(self.comprehensive_results, 'to_csv'):
            self.comprehensive_results.to_csv(f'{output_dir}/first_half_comprehensive_analysis.csv', index=False)
        else:
            print("No comprehensive results to export")
        ...
        with open(f'{output_dir}/first_half_comprehensive_report.json', 'w') as f:
            json.dump(report, f, indent=2, default=str)
        
        print(f"First half results exported to: {output_dir}")
        print(f"  Comprehensive analysis: {output_dir}/first_half_comprehensive_analysis.csv")
        print(f"  Individual windows: {output_dir}/window_XXX/")
        print(f"  Comprehensive report: {output_dir}/first_half_comprehensive_report.json")


def main():
    """
    Main function to run the first half sliding window analysis
    """
    print("First Half Sliding Window TDA Analysis")
    print("=" * 50)
    ...
    max_filtration = 2.0
    max_workers = 4       # Use 4 workers for stability
    ...
    # Initialize analyzer
    analyzer = FirstHalfSlidingWindowAnalyzer(
        data_file=data_file,
    ...
    # Create sliding windows for first half
    analyzer.create_first_half_windows()
    ...
    print("First half sliding window analysis completed successfully!")
    print("Check the 'first_half_sliding_window_results' directory for detailed results.")
