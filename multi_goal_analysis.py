...
class MultiGoalAnalysis:
    ...
    def compute_gps_aware_h0(
        ...
        Args:
            ...
            cutoff_distance: Cut-off distance in meters for clustering
            max_filtration: Maximum filtration value (uses self.max_filtration if None)
        ...
        # Step 3: Persistent homology on clustered point cloud
        if len(point_cloud) > 1:
            try:
                # Determine final filtration value
                # Priority: 1) function parameter, 2) instance variable, 3) adaptive
                final_filtration = max_filtration
                if final_filtration is None:
                    final_filtration = self.max_filtration
                
                # Adaptive filtration: Use point cloud distances if no filtration specified
                # This is critical for H1 detection after clustering with larger cut-offs!
                if final_filtration is None or final_filtration <= 0:
                    ...
                    if len(point_distances) > 0:
                        # Use 75th percentile of distances as max filtration (like previous implementation)
                        ...
                        final_filtration = max(5.0, cutoff_distance * 2.0)
