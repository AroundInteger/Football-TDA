#!/usr/bin/env python3
"""
First Half Efficient Sliding Window TDA Analysis
===============================================

This script analyzes the first half of the SecondSpectrum match using smaller
windows and more efficient point cloud creation to avoid memory issues.

Features:
- Smaller windows (2 minutes instead of 5)
- Reduced point cloud size
...
class FirstHalfEfficientAnalyzer:
    """
    Efficiently analyzes the first half using smaller windows and reduced data
    """
    ...
    def __init__(self, data_file, window_size=3000, step_size=600, max_filtration=1.5):
        ...
        print(f"FirstHalfEfficientAnalyzer initialized")
    ...
    def create_efficient_windows(self):
        """
        Create efficient window definitions for the first half
        """
        print(f"\n=== Creating Efficient First Half Windows ===")
        ...
        print(f"Created {len(self.windows)} efficient windows for first half")
    ...
    def analyze_all_windows_sequential(self):
        ...
        """
        print(f"\n=== Analyzing All Windows Sequentially ===")
        
        if not self.windows:
            self.create_efficient_windows()
    ...
    def export_results(self, output_dir='first_half_efficient_results'):
        """
...
def main():
    """
    Main function to run the efficient first half analysis
    """
    print("First Half Efficient Sliding Window TDA Analysis")
    print("=" * 50)
    
    # Configuration - smaller windows and reduced filtration
    data_file = "FieldTest/g2293068_SecondSpectrum_Data copy.txt"
    window_size = 3000    # 2 minutes at 25Hz (reduced from 5 minutes)
    step_size = 600       # 24 seconds step (reduced from 1 minute)
    max_filtration = 1.5  # Reduced from 2.0
    ...
    # Initialize analyzer
    analyzer = FirstHalfEfficientAnalyzer(
        data_file=data_file,
    ...
    # Create efficient windows
    analyzer.create_efficient_windows()
    ...
    print("\n=== Efficient First Half Analysis Complete ===")
    print("Efficient first half analysis completed successfully!")
    print("Check the 'first_half_efficient_results' directory for detailed results.")
