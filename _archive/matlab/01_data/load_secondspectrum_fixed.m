function [home_positions, away_positions, timestamps, metadata] = load_secondspectrum_fixed(filename, max_rows)
% LOAD_SECONDSPECTRUM_FIXED Load and parse SecondSpectrum JSON data correctly
% 
% Inputs:
        ...
        if ischar(line) && ~isempty(line)
            try
                ...
                if isfield(data, 'homePlayers') && iscell(data.homePlayers)
                    ...
                    for p = 1:n_home
                        if isfield(home_players{p}, 'xyz') && length(home_players{p}.xyz) >= 3
                            home_frame(p, :) = home_players{p}.xyz(1:3);
                        else
                    ...
                    if isfield(data, 'awayPlayers') && iscell(data.awayPlayers)
                        ...
                        for p = 1:n_away
                            if isfield(away_players{p}, 'xyz') && length(away_players{p}.xyz) >= 3
                                away_frame(p, :) = away_players{p}.xyz(1:3);
                            else
