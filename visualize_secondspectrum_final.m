% Visualize Real SecondSpectrum Data - Final Working Version
...
xlim([-50, 50]); ylim([-40, 40]); % Adjusted for actual data range
...
for p = 1:min(3, metadata.n_home_players)
...
% Plot 1: Position density for home team
subplot(2, 4, 1);
...
if sum(valid_positions) > 0
    scatter(home_x_all(valid_positions), home_y_all(valid_positions), 20, 'b', 'filled', 'MarkerFaceAlpha', 0.3);
    xlabel('X Position (m)'); ylabel('Y Position (m)');
    title('Home Team Position Density');
    xlim([-50, 50]); ylim([-40, 40]);
    grid on;
end

% Plot 2: Position density for away team
subplot(2, 4, 2);
...
if sum(valid_positions) > 0
    scatter(away_x_all(valid_positions), away_y_all(valid_positions), 20, 'r', 'filled', 'MarkerFaceAlpha', 0.3);
    xlabel('X Position (m)'); ylabel('Y Position (m)');
    title('Away Team Position Density');
    xlim([-50, 50]); ylim([-40, 40]);
    grid on;
end
...
% Save figures (fix the saveas error)
try
    saveas(gcf, 'secondspectrum_detailed_analysis.png');
    fprintf('Detailed analysis figure saved\n');
catch
    fprintf('Could not save detailed analysis figure\n');
end

try
    % Get the previous figure handle
    fig_handles = findall(0, 'Type', 'figure');
    if length(fig_handles) >= 2
        saveas(fig_handles(2), 'secondspectrum_overview.png');
        fprintf('Overview figure saved\n');
    end
catch
    fprintf('Could not save overview figure\n');
end

% Save data
output_dir = './secondspectrum_results';
...
fprintf('  ✓ Position density plots\n');
...
fprintf('\nThe real SecondSpectrum data has been successfully analyzed and visualized!\n');
