...
def find_best_cycle(points, birth, death):
    ...
    if not all_cycles:
        # Fallback: return nodes connected by edges in [birth, death]
        connected = set()
        for i in range(n):
            for j in adj[i]:
                connected.add(i)
...
# ── Figure 2: Temporal evolution data ──────────────────────────────────────
...
# ── Figure 3: Event correlation data ───────────────────────────────────────
    ...
    with open(OUT / 'fig3_event_correlation.csv', 'w', newline='') as f:
        ...
        for scale in ['individual', 'tactical']:
            for etype, vals in ec['statistical_tests'][scale].items():
                w.writerow([etype, scale, vals['n_events'], vals['mean_delta'],
...
# ── Persistence diagrams: birth/death for representative frames ──────────────
