...
Representative \(H_1\) points use the maximal-persistence loop per scale
(same framing as Fig.~2 export). Prefer ``04_h1_loops/.../h1_loops_skillcorner_1996435.json``;

Override with ``TDA_H1_LOOPS_JSON``.
"""
...
def main():
    data_file = loops_json_path()
    with open(data_file) as f:
        loops = json.load(f)

    best_tac = sorted(
        [l for l in loops if l['scale'] == 'tactical'],
        key=lambda x: x['persistence'],
        reverse=True,
    )[0]
    best_ind = sorted(
        [l for l in loops if l['scale'] == 'individual'],
        key=lambda x: x['persistence'],
        reverse=True,
    )[0]
