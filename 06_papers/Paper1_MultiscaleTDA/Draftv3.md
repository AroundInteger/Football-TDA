...
![Figure 1: Multi-scale TDA analysis pipeline.](figures/fig1_pipeline_schematic.png)
...
Figure 2 shows representative persistence diagrams at the individual and tactical scales for a single frame, illustrating the (birth, death) coordinates of H1 features above the diagonal.

![Figure 2: Representative persistence diagrams at individual and tactical scales.](figures/fig_persistence_diagrams.png)
...
![Figure 3: Geometric realisation of H1 loops at individual and tactical scales.](figures/fig2_cycle_geometry.png)
...
The tactical scale shows a statistically significant decrease in persistence from first to second half (p < 0.001), while the individual scale shows no significant change (Figure 4). This result differs in sign from our preliminary windowed analysis (which suggested an increase). The preliminary analysis used overlapping 2-minute windows, introducing temporal autocorrelation that inflated second-half persistence estimates; the definitive analysis uses non-overlapping windows with full-frame resolution per window, avoiding double-counting and autocorrelation. We therefore treat the non-overlapping, window-level analysis as definitive and report its results.
...
![Figure 4: Temporal evolution of H1 persistence across analysis windows.](figures/fig3_temporal_evolution.png)
...
Real event correlation using SkillCorner annotations across 10 matches (104,722 event-topology pairs) reveals statistically significant associations between match events and topological transitions (Figure 5):
...
![Figure 5: Persistence change by match event type.](figures/fig4_event_correlation.png)
